...
1. title — layout="title". title = тема доклада простыми словами, без канцелярита. ОБЯЗАТЕЛЬНО image_query на английском.
2. context — layout="bullets". title = конкретная формулировка "зачем это знать" (не "Контекст"), например "Где это встречается в жизни каждый день". subtitle = одно предложение. bullets: 2-3 пункта, где эта тема реально проявляется в практике/жизни аудитории.
3. concept1 — layout="bullets". title = конкретная формулировка первой части темы (не "Часть 1", не "Концепция 1") — реальное название идеи. bullets: объясни через логику и понятную бытовую аналогию, НЕ через цифры/статистику. 3-4 пункта.
4. example1 — layout="diagram" (если у идеи есть механизм/процесс — обязательно заполни mermaid_code) или "image_full" (если наглядность важнее процесса). title = конкретное название примера/механизма к concept1, НЕ "Пример 1".
5. concept2 — layout="bullets". title = конкретная формулировка второй части темы. Тем же простым языком, что и concept1. bullets: 3-4 пункта.
6. example2 — layout="quote" (запоминающийся факт или аналогия одной фразой в body_text, subtitle = откуда мысль) или "diagram". title = не оставляй пустым даже для quote-layout — короткая подпись темы цитаты.
7. common_mistakes — layout="bullets". title = конкретная формулировка ("Что обычно путают", "Три частые ошибки" — не "Ошибки"). bullets: 3-4 частых заблуждения или сложных момента по теме.
8. summary — layout="bullets". title = "Главное за 30 секунд" или похожая конкретная формулировка. bullets: 3-4 главные мысли доклада, сжато.
9. closing — layout="closing". title = конкретный призыв ("Попробуйте сами", "Куда копать дальше" — не "Вопросы"). body_text = что аудитории сделать после доклада: попробовать/почитать/спросить.

ВАЖНО: поле title заполняется НА КАЖДОМ слайде без исключения, включая quote и diagram — короткой, конкретной, не шаблонной формулировкой. Пустой или общий title ("Ключевые тезисы", "Часть 2") — ошибка.""",

    ContentSourceType.TEXT: """\
СТРУКТУРА ОТЧЁТНОГО ДОКЛАДА ПО ГОТОВОМУ МАТЕРИАЛУ (строго 9 слайдов):
1. title — layout="title". title = резюме проделанной работы одной строкой из материала (НЕ "Доклад", а конкретный итог — "Что сделано за квартал стажировки"). ОБЯЗАТЕЛЬНО image_query на английском.
2. context — layout="bullets" или "quote". title = конкретная формулировка задачи/цели ИЗ материала (не "Контекст"). bullets/body_text: какая была исходная задача.
3. done — layout="bullets". title = конкретная формулировка ("Что сделано", "Ключевые шаги" — можно точнее, если материал позволяет). bullets: конкретные пункты СТРОГО из присланного материала, не выдумывай.
4. results — layout="metrics" (если в материале есть цифры, с source = откуда цифра в материале) или "bullets" (если цифр нет — НЕ выдумывай их, просто опиши результат словами). title = конкретная формулировка результата.
5. results2 — layout="diagram" (если в материале описан процесс/механизм — заполни mermaid_code по этому процессу) или "bullets". title = конкретная формулировка дополнительного результата или процесса.
6. difficulties — layout="bullets". title = конкретная формулировка ("С какими сложностями столкнулись" — не "Проблемы"). bullets: 3-4 сложности из материала, без приукрашивания.
7. lessons — layout="quote". title = короткая подпись темы вывода (не пустая строка). body_text = главный вывод/урок одной фразой. subtitle = откуда вывод (автор материала, если известен).
8. next_steps — layout="timeline". title = "Следующие шаги" или конкретнее. timeline_items: 3-4 пункта плана из материала или логично вытекающих из него.
9. closing — layout="closing". title = конкретный призыв ("Готовы обсудить детали" — не "Вопросы"). body_text = итог и приглашение к вопросам.

ВАЖНО: поле title заполняется НА КАЖДОМ слайде без исключения, включая quote и diagram — короткой, конкретной, не шаблонной формулировкой. Пустой или общий title — ошибка.""",
